...
import com.sforce.cc.tools.data2sql.*;
...
public class MappingMemberOf
    ...
    public void checkConsistency() {
        ...
        Map<String, List<String>> missingMemberOf = new TreeMap<>();
        ...
        for (String dn : entrym.keySet()) {
            for (String um : entrym.get(dn).getUniqueMember()) {
                ...
                if (!entrym.get(um).getMemberOf().contains(dn)) {
                    ...
                    missingMemberOf.put(dn, missingMemberOf.getOrDefault(dn, new ArrayList<>()));
                    missingMemberOf.get(dn).add(um);
                }
            }
        }
        ...
        LOGGER.log(always, "                 existing uniqueMember : {}", entrym.values().
                stream().mapToInt(v -> v.getUniqueMember().size()).sum());

        LOGGER.log(always, "                   referenced memberOf : {}", entrym.values().
                stream().mapToInt(v -> v.getMemberOf().size()).sum());

        LOGGER.log(always, "                      missing memberOf : {}", missingMemberOf.values().
                stream().mapToInt(v -> v.size()).sum());

        if (!missingMemberOf.isEmpty()) {
            for (Map.Entry<String, List<String>> e : missingMemberOf.entrySet()) {
                LOGGER.log(always, "{} ({})", e.getKey(), e.getValue().size());
                LOGGER.log(always, "    --> entries with missing memberOf");
                e.getValue().forEach(s -> LOGGER.log(always, "    {}", s));
            }
        }
    }
}
